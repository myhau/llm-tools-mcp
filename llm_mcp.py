...
from .config import McpClient, McpConfig
import asyncio

def create_tool_for_mcp(mcp_client: McpClient, mcp_tool: mcp.Tool) -> llm.Tool:
    def impl(**kwargs):
        return asyncio.run(mcp_client.call_tool(mcp_tool.name, **kwargs))
    return llm.Tool(
        name=mcp_tool.name,
...
def register_tools(register):
    ...
    for (server_name, tools) in asyncio.run(mcp_client.get_all_tools()).items():
        for tool in tools:
            register(create_tool_for_mcp(mcp_client, tool))
...
class McpConfig:
    def __init__(self, path = "~/.cursor/mcp.json"):
...
class McpClient:
    ...
    def server_params_for(self, name: str):
        server_config = self.config.get().servers.get(name)
